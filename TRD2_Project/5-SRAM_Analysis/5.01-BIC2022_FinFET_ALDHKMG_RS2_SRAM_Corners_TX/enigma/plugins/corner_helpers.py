...
def PCA_Points(xdata, ydata, sigma_extr=3):
    ...
    for x,axis in enumerate(eigenvectors):
        start1, end1 = mu , mu + (sigma[x] * axis * sigma_extr)*std
        start2, end2 = mu - (sigma[x] * axis *sigma_extr)*std , mu 
...
class DoESurface(object):
    ...
    def __init__(self, df, corner=None, err_scaling_list=None, verbose=True, a_scale_factors=None, target_err=1e-3):
        ...
        for col in df.columns:
            df2=df[col].dropna()
            self._interps[col] = interp.LinearNDInterpolator(df2.index.tolist(), df2.values.tolist(), fill_value=1e9, rescale=True)
