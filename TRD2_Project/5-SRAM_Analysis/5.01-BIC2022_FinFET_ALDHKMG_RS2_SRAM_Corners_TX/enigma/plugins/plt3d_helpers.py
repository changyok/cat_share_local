...
def multiDimSurfaceMatrix(df_in, axes_labels, param, sampling=11,fontsize=20, outpath="./",prefix="node", errorplot=False, errorlims=10):

    if len(axes_labels) < 3:
        #print("axes < 3, useing traditional potting methods")
        multiplot_param_across_doe(df_in, devtype='', parameters=param, xaxis=axes_labels[0], yaxis=axes_labels[1], fontsize=20, save_plot=True, device_or_circuit='circuit', pathprefix=outpath, cmap=True, zlabel=None, file_prefix=prefix, sampling=sampling, errorlims=errorlims)
    ...
    for axes in permutations(axes_labels):
        ...
        if set(list(axes[:3])) not in plotted_axes:
            ...
            plt.show()
            plt.savefig(f"{prefix}{a1}_{a2}_{a3}_{param}.png")
            plt.close()
