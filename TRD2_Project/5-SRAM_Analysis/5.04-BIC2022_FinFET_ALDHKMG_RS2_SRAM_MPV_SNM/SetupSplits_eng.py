...
Mystic_path = "@pwd@/../../3-SpiceModel/3.06-BIC2022_FinFET_ALDHKMG_Mystic_SRAM_Corners"
startdb(Mystic_path)
...
hfin_nom = nodes[0].metadata["swb"]["hfin"]
...
for t in [highT]:
    add_experiments(tree, init_nodes, {"axis1_midpoint":f"{l_nom:.5}","axis2_midpoint":f"{tfin_nom:.5}",  
                                       "Vdd_nom":f"{Vdd_nom:.5}", "sim_temp":t}, scenario="default")
